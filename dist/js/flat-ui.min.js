...
String.prototype.repeat=function(a){return Array(Math.round(a)+1).join(this)};
(function(a){a.fn.addSliderSegments=function(){return this.each(function(){var b=a(this),c=b.slider("option"),d=(c.max-c.min)/c.step;if("vertical"===c.orientation){var c="",e;console.log(d);for(e=1;e<=d-1;e++)c+='<div class="ui-slider-segment" style="top:'+100/d*e+'%;"></div>';b.prepend(c)}else b.prepend(('<div class="ui-slider-segment" style="margin-left: '+(100/d+"%")+';"></div>').repeat(d-1))})};a(function(){a(".todo").on("click","li",function(){a(this).toggleClass("todo-done")});a('[data-toggle="select"]').length&&
a('[data-toggle="select"]').select2();a('[data-toggle="checkbox"]').radiocheck();a('[data-toggle="radio"]').radiocheck();a("[data-toggle=tooltip]").tooltip("show");var b=a("#slider");0<b.length&&b.slider({max:15,step:6,value:3,orientation:"horizontal",range:"min"}).addSliderSegments();b=a("#vertical-slider");b.length&&b.slider({min:1,max:5,value:3,orientation:"vertical",range:"min"}).addSliderSegments(b.slider("option").max,"vertical");a(".input-group").on("focus",".form-control",function(){a(this).closest(".input-group, .form-group").addClass("focus")}).on("blur",
    ".form-control",function(){a(this).closest(".input-group, .form-group").removeClass("focus")});a(".pagination").on("click","a",function(){a(this).parent().siblings("li").removeClass("active").end().addClass("active")});a(".btn-group").on("click","a",function(){a(this).siblings().removeClass("active").end().addClass("active")});a(document).on("click",'a[href="#fakelink"]',function(a){a.preventDefault()});a('[data-toggle="switch"]').length&&a('[data-toggle="switch"]').bootstrapSwitch();a("#typeahead-demo-01").length&&
(b=new Bloodhound({datumTokenizer:function(a){return Bloodhound.tokenizers.whitespace(a.word)},queryTokenizer:Bloodhound.tokenizers.whitespace,limit:4,local:[{word:"Alabama"},{word:"Alaska"},{word:"Arizona"},{word:"Arkansas"},{word:"California"},{word:"Colorado"}]}),b.initialize(),a("#typeahead-demo-01").typeahead(null,{name:"states",displayKey:"word",source:b.ttAdapter()}));window.prettyPrint&&prettyPrint()})})(jQuery);
